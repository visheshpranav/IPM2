import re,os,xmltodict,json,sys
import cgi, cgitb 
...
def generateUsage(service_arr,userid,rep_id,rep_name,appName,Imp_name,startdate,enddate,mode,search):
    if mode == "OFL":
        ...
        if not os.path.exists("../../../WEB-INF/classes/static/html/Reports/"+rep_name1):
            os.mkdir("../../../WEB-INF/classes/static/html/Reports/"+rep_name1)
    ...
    if mode == "OFL":
        response_arr = []
        filepath = "../../../WEB-INF/classes/static/html/Reports/"+rep_name1+"/"
...
def generateDC(userid,rep_id,report_name,appName,Imp_name,startdate,enddate,mode,search):
    ...
    if mode == "OFL":
        filepath = "../../../WEB-INF/classes/static/html/Reports/"+rep_name+"/"
...
##    mode = sys.argv[6]
##    Imp_name = sys.argv[9]
##    start_time = sys.argv[10].strip().replace('T', ' ')
##    end_time = sys.argv[11].strip().replace('T', ' ')
##    search = sys.argv[12]
