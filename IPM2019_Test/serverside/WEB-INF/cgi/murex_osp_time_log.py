# -*- coding: utf-8 -*-
"""
Created on Sun Feb 28 22:14:07 2021
...
def get_dict(file_path, c_dict, cid_time_dict, typology_dict, master_df):
    ...
    for index, row in data.iterrows():
        ...
        if row["CONTRACT_ID"] in c_dict:
            ...
            cid_time_dict[row["CONTRACT_ID"]].append(row['Entry Time'])
            
        else:
            c_dict[row["CONTRACT_ID"]]=[]
            cid_time_dict[row["CONTRACT_ID"]] = []
            c_dict[row["CONTRACT_ID"]].append(row["OSP_QUEUE_NAME"]+":"+row["FROM_TASK"]+":"+row["TO_TASK"])
            cid_time_dict[row["CONTRACT_ID"]].append(row['Entry Time'])
##    print(c_dict)
##    print("--------------------------------")
##    print(cid_time_dict)
    ...
    return c_dict, cid_time_dict, typology_dict, master_df

def type_def(c_dict, cid_time_dict, typology_dict, time_obj):
    type_dict = dict()
    dendo_dict = dict()
    type_cid_dict = dict()
    dist_type_dict = dict()
    ...
    for typo, c_list in typology_dict.items():
        ...
        for cid in c_list:
            flag = 1
            cid_type_list = []
            start_time = cid_time_dict[cid][0]
            end_time = cid_time_dict[cid][-1]
            diff = end_time - start_time
            #print(diff,time_obj)
            if diff > time_obj:
                single_set = []
                for i in range(len(cid_time_dict[cid])):
                    if not start_time:
                        start_time = cid_time_dict[cid][i]
                    if (cid_time_dict[cid][i]-start_time) > time_obj:
                        start_time = cid_time_dict[cid][i]
                        cid_type_list.append(single_set)
                        single_set = [c_dict[cid][i]]
                    else:
                        single_set.append(c_dict[cid][i])
                        #print(single_set)
                cid_type_list.append(single_set)
            else:
                cid_type_list.append(c_dict[cid])
            #print(cid_type_list)
            for val in cid_type_list:
                for type_id, type_set in type_dict.items():
                    ...
                    if val == type_set:
                        ...
                        if type_id not in dendo_dict[typo]:
                            ...
                            val_list = [1 for val in range(len(val))]
...
def generate_dendo(dendo_dict, type_cid_dict, master_df, output_path, del_path):
    dendo_list = []
    final_list = []
    ...
    dev_master_df = pd.read_csv(del_path+'//Del_master.csv')
    ...
    master_df = master_df.join(dev_master_df.set_index('CONTRACT_ID'),on=['CONTRACT_ID'])
...
if __name__=="__main__":
    ...
    test_name = form.getvalue('NReportname')
    org_test_name = form.getvalue('OReportname')
    resp_time = float(form.getvalue('resp_time')) * 60
##    resp_time = int(form.getvalue('resp_time')) * 60
    # logtype = form.getvalue('logtype').split("\n")[1].strip()
##    userid="44"
##    test_name="OTB_war"
##    org_test_name = "testospdrpbx"
##    resp_time = 20.400000000000002
    logtype="OSP"
    dir_path = DATA_PATH + "Cache\\" + userid + "\\" + org_test_name+"\\"+str(logtype)
    master_path = DATA_PATH + "Cache\\" + userid + "\\" + org_test_name+"\\DeliverableMXML"
    ...
    time_obj = datetime.timedelta(minutes = resp_time)
    # print(time_obj)
    # print('================================')
    c_dict=dict()
    cid_time_dict = dict()
    ...
    for file in files:
        c_dict, cid_time_dict, typology_dict, master_df=get_dict(file, c_dict, cid_time_dict, typology_dict, master_df)
    # print(cid_time_dict)
    dendo_dict, type_dict, type_cid_dict = type_def(c_dict, cid_time_dict, typology_dict, time_obj)
    # print(type_cid_dict)
    folder_name=userid+"_"+test_name
    del_folder_name = userid+"_"+org_test_name
    del_path = "../../../WEB-INF/classes/static/html/Reports/"+del_folder_name
    ...
    generate_dendo(dendo_dict, type_cid_dict, master_df, output_path, del_path)
    print("success")
