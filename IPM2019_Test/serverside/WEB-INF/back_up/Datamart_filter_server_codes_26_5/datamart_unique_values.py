...
##    test_name = "dmreptab"
    ...
    datamart_output = pd.read_csv(output_path + '\\Datamart_output.csv', dtype=str)
    ...
    datamart_df['TIME'] = datamart_df['TIME'].astype('str')
    if column_name == 'COLUMNS':
        column_details_df = column_details_df.loc[column_details_df['TABLE_NAME'].isin(datamart_df['REPORT_LABEL'].unique())]
        unique_values = list(column_details_df['COLUMNS'].unique())
    else:
        unique_values = list(datamart_df[column_name].unique())
        unique_values = [val for val in unique_values if val in datamart_output.values]
