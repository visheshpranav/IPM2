# -*- coding: utf-8 -*-
"""
Created on Fri Mar 19 11:47:57 2021

@author: v.a.subhash.krishna
"""

import pandas as pd
import string,cgi, glob, os,sys
...
def dist_type(type_list):
    ...
    for i in range(len(unique_list)):
        seq_val = unique_list[i]
        for val in range(type_list_counter,type_list_counter+seq_val[1]):
            if type_list[val].split('!@#')[0] in queue_count and val==type_list_counter:
                queue_count[type_list[val].split('!@#')[0]] += 1
                final_queue_name = type_list[val].split('!@#')[0]+'_'+str(queue_count[type_list[val].split('!@#')[0]])
                final_list.append(final_queue_name+'!@#'+'!@#'.join(type_list[val].split('!@#')[1:]))
            else:
                if val == type_list_counter:
                    final_list.append(type_list[val])
                    queue_count[type_list[val].split('!@#')[0]] = 1
        type_list_counter+=seq_val[1]
        val_list.append(seq_val[1]) 
    ...
    # print(len(final_list),len(val_list))
    # sys.exit(1) 
    return [final_list,val_list]
...
def get_dict(file_path, c_dict, typology_dict, master_df):
    ...
    data_columns = [' '.join(val.split()) for val in data_columns]
    data.columns = data_columns
    master_df = pd.concat([master_df,data])
    #data = pd.read_csv("dendo_new.csv")
    req_columns = ["TYPOLOGY", "CONTRACT_ID", "OSP_QUEUE_NAME", "FROM_TASK", "TO_TASK", "ENTRY TIME"]
    ...
    data=data.sort_values(['TYPOLOGY'], ascending=True) \
        ...
        .apply(lambda x: x.sort_values(['ENTRY TIME','OSP_QUEUE_NAME',"FROM_TASK", "TO_TASK"], ascending=True)) \
...
def type_def(c_dict, typology_dict):
    ...
    for typo,c_list in typology_dict.items():
        if len(type_dict):
            start_type = list(type_dict.keys())[-1]
        c_list = list(c_list)
        dendo_dict[typo] = dict()
        for cid in c_list:
            flag = 1
            type_id = [key for key,value in type_dict.items() if c_dict[cid] == value]
            # for type_id, type_set in type_dict.items():
                # pos = is_sub(c_dict[cid], type_set)
            # if c_dict[cid] == type_set:
            if type_id:
                flag = 0
                if type_id[0].split(' ')[1] not in type_cid_dict:
                    type_cid_dict[type_id[0].split(' ')[1]] = []
                type_cid_dict[type_id[0].split(' ')[1]].append(cid)
                if type_id[0] not in dendo_dict[typo]:
                    dendo_dict[typo][type_id[0]] = []
                    dendo_dict[typo][type_id[0]].extend(dist_type_dict[type_id[0]])
                    # val_list = [1 for val in range(len(c_dict[cid]))]
                    # dendo_dict[typo][type_id[0]].append(val_list)
                    # for val in c_dict[cid]:
                    #     dendo_dict[typo][type_id][val] = 1
                else:
                    val_list = []
                    for i in range(len(dendo_dict[typo][type_id[0]][1])):
                        val_list.append(dist_type_dict[type_id[0]][1][i]+dendo_dict[typo][type_id[0]][1][i])
                    # val_list = [val+1 for val in dendo_dict[typo][type_id[0]][1]]
                    dendo_dict[typo][type_id[0]][1] = val_list
                    # for val in c_dict[cid]:
                    #     dendo_dict[typo][type_id][val]+=1
                # break
            ...
            if flag:
                ...
                dendo_dict[typo][start_type].extend(dist_type_dict[start_type])
                # val_list = [1 for val in range(len(c_dict[cid]))]
                # dendo_dict[typo][start_type].append(val_list)
    return dendo_dict, type_dict, type_cid_dict

def generate_dendo(dendo_dict, type_cid_dict, master_df, output_path, test_name):
    dendo_list = []
    final_list = []
    ...
    if os.path.exists(output_path+'//Del_master.csv'):
        dev_master_df = pd.read_csv(output_path+'//Del_master.csv')
        if not dev_master_df.empty:
    ...
    for each_type,cid_list in type_cid_dict.items():
        master_df.loc[master_df['CONTRACT_ID'].isin(cid_list), 'TYPE'] = 'Type '+each_type
    ...
    # print('get_typology completed - ',datetime.now())
    ...
    with pd.ExcelWriter(output_path+'//'+test_name+'_OSP_Master.xlsx') as writer:
...
def get_typology(master_df, output_path):
    ...
    for typo in unique_typology:
        ...
        final_list_1.append([typo,count])
        types = list(df['TYPE'].unique())
        for _type in types:
            df1 = df.loc[df['TYPE'] == _type]
            count = len(df1['CONTRACT_ID'].unique())
            final_list_2.append([typo,_type,count])
    df = pd.DataFrame(final_list_1,columns=['Typology','Count'])
    df.to_csv(output_path+'//OSP_Typology.csv',index=False, header=True)
    df1 = pd.DataFrame(final_list_2,columns=['Typology','Type','Count'])
    df1.to_csv(output_path+'//OSP_Type_Typology.csv',index=False, header=True)
...
##    test_name="testospp"
    ...
    file_path = dir_path + "\\*"
    # file_path = 'C:\\Users\\v.a.subhash.krishna\\Desktop\\Deliverable\\OSP_input\\trim_rambo_data.xlsx'
    # master_path = ''
    # test_name="sample"
    ...
    # print('get_dict completed - ',datetime.now())
    dendo_dict, type_dict, type_cid_dict = type_def(c_dict, typology_dict)
    # print(len(type_dict))
    # print('type_def completed - ',datetime.now())
    # print(type_cid_dict)
    folder_name=userid+"_"+test_name
    output_path = "../../../WEB-INF/classes/static/html/Reports/"+folder_name
    # output_path = 'C:\\Users\\v.a.subhash.krishna\\Desktop\\Deliverable\\OSP_Rabo_Output_2\\'
    if not os.path.exists(output_path):
        os.mkdir(output_path)
##    print('get_typology completed - ',datetime.now())
    generate_type_csv(type_dict, type_cid_dict,typology_dict, master_path, output_path)
##    print('generate_type_csv completed - ',datetime.now())
    generate_dendo(dendo_dict, type_cid_dict, master_df, output_path, test_name)
