	...
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {				
			...
			{		
				...
				if(localStorage.getItem("ipm_appname").toLowerCase() == "murex")
				{
					...
					txtFile.onreadystatechange = function() {
						if (this.readyState == 4 && this.status == 200)
						{
							...
							alert("Pattern matching failed, try again");
							return false;
							}*/
				...
				{
				delay(function(){
				alert("Pattern Mined from the Log - Patterns are matched")
				hideLoader();
				}, 8000 );
				}
				var foldername= localStorage.getItem("ipm_UserId") + "_"+ localStorage.getItem("reportname")
				var filename2 = "Reports/"+foldername+"/IMP035_usage.csv";
				var http = new XMLHttpRequest();
				http.open('HEAD', filename2, false);
				http.send();
				if (http.status === 200){
				alert("Pattern Mined from the Log - Patterns are matched")
				hideLoader();
...
function pollDOM() {
	if(document.body.contains(document.getElementById('slt_trade')))
	{
      
    console.log("hi")
	if (document.getElementById("slt_trade").value=="All") {
		alert("Pattern Mined from the Log - Patterns are matched")
		hideLoader();
	} else {
