...
def compare_reports_master(base_master_df,compare_master_df,cols):
...
##    template_name = "TRD_Spotnomitemplate"
##    base_name = "rtrade8"
##    compare_name = "Rtrade9"
##    comparison_reportname = "Freshcompare"
    ...
    base_master_df = pd.read_csv(base_path+'/murex_master.csv')
    compare_master_df = pd.read_csv(compare_path+'/murex_master.csv')
...
##    print(len(compare_master_df))
    result_df = compare_reports_master(base_master_df,compare_master_df,cols)
    cols.extend(['Category','tradeFamily','tradeGroup','typology','Trade Characteristics','Contract Id','tradeInternalId'])
    cols = list(set(cols))
    generate_dendo(result_df,compare_output_path,cols)
