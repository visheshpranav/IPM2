...
##    df.to_csv("bar_graph.csv", index=False)
    ...
    for qname in queue_name:
        ...
        for typology in typology_list:
            if final_df_list[i][j] != '':
                dict1['Queue'] = "    "+qname+"    "
                dict1['name'] = typology
                dict1['count'] = final_df_list[i][j]
                if typology_dict[typology]:
                    dict1['Family'] =  typology_dict[typology]['Family']
                    dict1['Group'] =  typology_dict[typology]['Group']
                queue_list.append(dict1.copy())
            i=i+1
        j=j+1
    print(queue_list)
...
##    print(out)
    ...
    df.to_csv(output_path+"/"+"OSP_queuecount.csv", index=False)
