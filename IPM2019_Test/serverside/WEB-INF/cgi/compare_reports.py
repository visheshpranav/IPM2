...
def compare_reports_master(base_master_df,compare_master_df,
                           base_filter_master_df,compare_filter_master_df,cols):
    result_df = pd.DataFrame(columns=base_master_df.columns)
    result_filter_df = pd.DataFrame(columns=base_filter_master_df.columns)
    unique_combination_df = pd.DataFrame(columns=base_master_df.columns)
    base_groupby_df = base_master_df.groupby(cols)
    base_filter_groupby_df = base_filter_master_df.groupby(cols)
    missing_cols = [val for val in cols if val not in compare_master_df.columns]
##    print(event_values)
    for column in missing_cols:
        compare_master_df[column] = ''
        compare_filter_master_df[column] = ''

    compare_groupby_df = compare_master_df.groupby(cols)
    compare_list = list(dict(compare_groupby_df.groups).keys())
    compare_filter_groupby_df = compare_filter_master_df.groupby(cols)
    compare_filter_list = list(dict(compare_filter_groupby_df.groups).keys())
##    print(pos)
##    print(compare_list)
...
##            print(b_row[0])
##            print(b_row[1].columns)
            result_df = result_df.append(b_row[1])
            b_row[1].reset_index(drop=True,inplace=True)
##            print(b_row[1].iloc[0])
            unique_combination_df = unique_combination_df.append(b_row[1].iloc[0])
    for b_row in base_filter_groupby_df:
        if b_row[0] not in compare_filter_list:
            result_filter_df = result_filter_df.append(b_row[1])

    return result_df,unique_combination_df,result_filter_df
...
##        template_name = "TRD_Typo_Gen_MR"
##        base_name = "ZZZZ19"
##        compare_name = "ZTST3"
##        comparison_reportname = "Testcomparefreshdepl"
        ...
        if template_name:
            ...
            for key,val in template_json.items():
                ...
                if 'Events' in header_list:
                    pos = header_list.index('Events')
                    header_list[pos] = 'lastContractEventReference'
        ...
        try:
            ...
            base_filter_master_df = pd.read_csv(base_path+'/filter_murex_master.csv',keep_default_na = False)
        except:
        ...
        try:
            ...
            compare_filter_master_df = pd.read_csv(compare_path+'/filter_murex_master.csv',keep_default_na = False)
        except:
...
##        print(cols)
    ##    print(len(compare_master_df))
        result_df,unique_combination_df,result_filter_df = compare_reports_master(base_master_df,compare_master_df,
                                                                                  base_filter_master_df,compare_filter_master_df,
                                                                                  cols)
        if result_df.empty:
            raise GeneralException('Base Report and Compare Report are identical.')
##        print(base_master_df.columns,compare_master_df.columns)
        if 'tradeType' in result_df.columns:
            cols.extend(['Category','tradeFamily','tradeGroup','tradeType','Trade Characteristics','Contract Id','tradeInternalId'])
            cols = list(set(cols))
            generate_dendo(result_df,compare_output_path,cols,comparison_reportname,'Compare',unique_combination_df)
        else:
            cols.extend(['Category','tradeFamily','tradeGroup','Typology','Trade Characteristics','Contract Id','tradeInternalId'])
            cols = list(set(cols))
            generate_dendo_old(result_df,compare_output_path,cols,comparison_reportname,'Compare',unique_combination_df)
##        generate_dendo(result_df,compare_output_path,cols,comparison_reportname,'Compare')
        ...
        result_filter_df.to_csv(compare_output_path+'/filter_murex_master.csv', index=False, header=True)
        # print(len(result_df))
