...
def generateCSV_IMP003(matched_patterns1,userid,rep_id,Outcome1,report_name,loglines,Logic,mode,appName,time_main,date_time,Imp_name,logtype):                    
    ...
    if Logic == "None":
        ...
        if logtype == "Standard" or logtype == "WebSphere":
    ...
    for Outcome in Outcomes:
        if Outcome == "csv":
            ...
            for i in l:
                failurefile.write(i+"\n")
            failurefile.close()
            shutil.move(filename, "../../../WEB-INF/classes/static/html/"+move_path+"/"+filename)
            if mode == "ONL":
                shutil.copy("../../../WEB-INF/classes/static/html/Online/"+filename,newfile)
...
if __name__ == "__main__":
    ...
    matched_patterns = "P3,P4,P5,P6,P7,P8,P10,P23,P27,P25,P26,P64"
    userid = "9"
    rep_id = "741"
    rep_name = "a6"
    Outcome = "csv"
    Logic = "Customization"
    logpath_file = "C:\\LAPAM2019\\DataLake\\Logs\\Cache\\9\\a6\\WebSphere\\SystemOut.log"
    mode = "OFL"
    appName = "TWA"
