    ...
    for typo,c_list in typology_dict.items():
        ...
        for cid in c_list:
            ...
            for type_id, type_set in type_dict.items():
                ...
                if c_dict[cid] == type_set:
                    ...
                    if type_id not in dendo_dict[typo]:
                        ...
                        dendo_dict[typo][type_id].append(c_dict[cid])
            ...
            if flag:
                ...
                dendo_dict[typo][start_type].append(c_dict[cid])
                val_list = [1 for val in range(len(c_dict[cid]))]
                dendo_dict[typo][start_type].append(val_list)
    return dendo_dict, type_dict, type_cid_dict

def generate_dendo(dendo_dict, master_df, output_path):
    dendo_list = []
...
def generate_type_csv(type_dict, type_cid_dict, master_path, output_path):
    type_list = []
    unique_queue = dict()
    ...
    master_data(output_path, master_path, type_cid_dict, unique_queue)
...
##    test_name="testospp"
    ...
    generate_type_csv(type_dict, type_cid_dict, master_path, output_path)
    generate_dendo(dendo_dict, master_df, output_path)
