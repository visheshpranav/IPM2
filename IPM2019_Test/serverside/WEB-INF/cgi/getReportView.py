...
##    print(res)
    ...
    for doc in agg_val:
        ...
        for get_details in doc['user_count']['buckets']:
            max_user_dict[get_details['key']] = get_details['max_user_res']['value']
            # maximum_user = get_details['key']
        ...
        if 'ZJANUS_BGR_' in doc['key']:
            ...
            elif 'sap/MD_BUSINESSP' in doc['key']:
                search_value = 'SAPMHTTP_MD_BUSINESSPARTNER_SRV'
            elif 'sap/ESH_SEARCH_S' in doc['key']:
                search_value = 'SAPMHTTP_ESH_SEARCH_SRV'
            elif 'sap/CV_ATTACHMEN' in doc['key']:
            ...
            search_value = 'RPC'
        elif 'ARFC' in doc['key']:
            search_value = 'ARFC'
        else:
            csvtcode_data.append(doc['key'] + "," + transaction_type + "," + str(len(doc['user_count']['buckets'])) + "," + str(len(impacted_user_count)) + ","+ str(doc['doc_count']) + "," + str(Failed_tran_count)+"," + str(round(doc['avg_res']['value'], 2)) + "," + str(maximum_user) + "," + str(round(doc['max_res']['value'], 2)))
            data = data + (doc['key'] + "," + transaction_type + "," + str(len(doc['user_count']['buckets'])) + "," + str(len(impacted_user_count)) + ","+ str(doc['doc_count']) + ","
                           + str(Failed_tran_count)+"," + str(round(doc['avg_res']['value'], 2)) + "," + str(maximum_user) + ","
                           + str(round(doc['max_res']['value'], 2))) + ","
        if search_value:
            ...
            for key in max_user_dict:
                if key not in tcode_users[search_value][0]:
                    ...
                    tcode_users[search_value][0][key] = max_user_dict[key] if max_user_dict[key]>tcode_users[search_value][0][key] else tcode_users[search_value][0][key]
            ...
            groupby_tcode_dict[search_value][5] += round(doc['avg_res']['value'], 2)
            groupby_tcode_dict[search_value][6] = str(maximum_user)
            groupby_tcode_dict[search_value][7] += round(doc['max_res']['value'], 2)
