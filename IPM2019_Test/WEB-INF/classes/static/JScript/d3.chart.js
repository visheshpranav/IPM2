...
define(['moment', 'underscore', 'd3.chart.analog', 'd3.chart.digital', 'd3.chart.horizon'], function (moment) {
    ...
    var gap = 25;
    var color = d3.scale.category10().range(["#A569BD", "#5499C7", "#F1C40F", "#808000"]);
                ...
                if (mX > 0 && mY > 0 && mX < width) { 
                    ...
                    if (nearestDateVal!=null) {
                        ...
                        d3.selectAll('.graph').data(_graphs, function (d) { return d.id; }).each(function (d) {
							...
							//console.log(d.id);
							if(d.id == "Used Tenured(After)")
							{
								g.select('.legend').text(d.id + ' : ' + str + ' GB');
							}
							else if((d.id == "User")  || (d.id == "Total") || (d.id == "Steal") || (d.id == "Nice"))
							{
								g.select('.legend').text(d.id + ' : ' + str + ' %');
							}
							else if(d.id == "Free Tenured(After)")
							{
								g.select('.legend').text(d.id + ' : ' + str + ' GB');
							}
							else if(d.id == "Free Nursery(After)")
							{
								g.select('.legend').text(d.id + ' : ' + str + ' MB');
							}							
							else if(d.id == "GC Completed")
							{
								g.select('.legend').text(d.id + ' : ' + str + ' ms');
							}
							else{
								g.select('.legend').text(d.id + ' : ' + str);
							}
    ...
    this.render = function () {
        ...
        //x-axis
        var xAxis = d3.svg.axis().scale(_xScale).orient("top").ticks(10);
