...
function bindUserDetail()
{
	...
	if(localStorage.getItem("ipm_appname").toLowerCase() === "murex")
	{
		...
		document.getElementById("addimp").style.display = "block";
		...
		document.getElementById("addimp").style.display = "none";
		...
		for (var file_i = 0; file_i < count_val.length; file_i++) {			
			...
			if(document.getElementById(file_id).value == "")
			{
				...
				if (document.getElementById(file_id).value.lastIndexOf(".") > 0) {
					...
					if (fileExtension.toLowerCase() == "csv" || fileExtension.toLowerCase() == "xls" || fileExtension.toLowerCase() == "xlsx" || fileExtension.toLowerCase() == "zip" || fileExtension.toLowerCase() == "tar" || fileExtension.toLowerCase() == "gz" || fileExtension.toLowerCase() == "log" ||fileExtension.toLowerCase() == "txt" ||fileExtension.toLowerCase() == "xml"||fileExtension.toLowerCase() == "001") {
						return true;
					}
					else {
...
function bulk_files(){
		...
		{
			alert("You cant submit, add filename field and enter the value");
			return false;
		}
		/*console.log($("#Text1").val());
		if($("#Text1").val() == "");
		{
			alert("You cant submit, provide filename which you uploaded in s3");
			return false;
		}*/
		 if(document.getElementById("Txt_reportname1").value == "")
		 {
			alert("You cant submit, provide reportname which you uploaded in s3");
			return false; 
		 }
		if(localStorage.getItem("ipm_appname").toLowerCase() === "murex")
		{
		...
		var jsonObj3=[];
		var jsonObj4=[];
		//var reportname = document.getElementById("Txt_reportname1").value;
		//console.log(reportname);
		//localStorage.setItem("report_name",reportname);
		//localStorage.setItem("reportname",reportname);
		//fd.append("Txt_reportname1",reportname);
		...
		{			
			...
			if(localStorage.getItem("ipm_appname") == "P002")
			{
				...
				{	*/	
						...
						for(var c=1;c<=e;c++)
						{
						var g= document.getElementById("Text"+c).value.trim();
						var type = document.getElementById("Logtype").value.trim();
						log_type = type + "-" + g;
						jsonObj2.push(g);
						jsonObj4.push(log_type);
						}
						
						/*var w= $('.folders').length;

						for(var k=1;w<=k;k++)
						{
						//var q=	document.getElementById("Txt_reportname1").value.trim();
						var q= document.getElementById("Txt_reportname"+k).value.trim();

						jsonObj3.push(q);

						}*/
					var type = document.getElementById("Logtype").value.trim();
					var filename = document.getElementById("Text1").value.trim();
					log_type = type + "-" + filename;
					//var filevalue = "C:/LAPAM2019/DataLake/Cache/44/"+reportname+"/"+type+"/"+filename;
					/*if(filevalue)
					{
						console.log("fsdf");
					}
					//var file_id = "Dlg_SMfile_" + 0
					//console.log(filevalue.files)
					var tra= new file(filevalue);
					console.log(tra);*/
					fd.append("filenames", jsonObj2);
					var reportname = document.getElementById("Txt_reportname1").value;
					console.log(reportname);
					localStorage.setItem("report_name",reportname);
					localStorage.setItem("reportname",reportname);
					//console.log(reportname);
					fd.append("Txt_reportname",reportname)
					//fd.append("Txt_reportname",jsonObj3);
					//localStorage.setItem("reportname",jsonObj3);
					fd.append("Log_type", jsonObj4);
					bulkeee=true;
					localStorage.setItem("logtype_filename",log_type);
					xhttp.open("POST", "../cgi-bin/bulk_upload.py", true);
				//}
			}
		}
...
function getlargefile()
{
	...
	document.getElementById('Btn_upload').disabled = false;
	document.getElementById("div_online_page").style.display = "block";	
	document.getElementById("div_online_page").innerHTML = '<object type="text/html" style = "width:100%; height:700px;" data="Large.html" ></object>';
...
function getaddimp()
{
	...
	document.getElementById('Btn_upload').disabled = false;
	document.getElementById("div_online_page").style.display = "block";	
	document.getElementById("div_online_page").innerHTML = '<object type="text/html" style = "width:100%; height:700px;" data="addimp.html" ></object>';	
...
function gethierarchy()
{
	...
	document.getElementById('Btn_upload').disabled = false;
	document.getElementById("div_online_page").style.display = "block";	
	document.getElementById("div_online_page").innerHTML = '<object type="text/html" style = "width:100%; height:700px;" data="hierarchy.html" ></object>';	
...
function add_fields(outputVal){
	...
	if(localStorage.getItem("ipm_appname") == "BOT")
	{
		...
		document.getElementById("Text1L").style.display = "none";
		...
		for (var file_i = 0; file_i < count_val.length; file_i++) {
			...
			if(localStorage.getItem("ipm_appname").toLowerCase() === "murex")
			{
				...
				additional_inputbtnfile.setAttribute("style","text-decoration:underline; cursor: pointer; margin-left:16px;");
				...
				additional_inputbtnfile2.setAttribute("style","margin-left:-34px;");
				additional_inputbtnfile2.setAttribute("onclick", "myFunction6('"+file_id+"')");
				var additional_inputbtnfile3 = document.createElement('label');
				additional_inputbtnfile3.innerHTML = "Auto";
				document.getElementById("Div_Addditional").appendChild(additional_inputbtnfile2);
				document.getElementById("Div_Addditional").appendChild(additional_inputbtnfile3);
				...
				document.getElementById("Text1L").style.display = "block";
			}
			else{
			...
			document.getElementById(lbl_id).style.cssFloat = "left";
			document.getElementById(lbl_id).style.width = "150px";
			document.getElementById(file_id).style.marginLeft = "8px";
			document.getElementById(file_id).style.cssFloat = "left"; 
			document.getElementById(file_id).style.top = '10px';
			document.getElementById(file_id).style.fontFamily = "AccentureRotis";
...
function switchVisible() {
            if (document.getElementById('Div_NewQual')) {

                if (document.getElementById('Ordinary').style.display == 'none') {
                    document.getElementById('Ordinary').style.display = 'block';
                    document.getElementById('Bulkd').style.display = 'none';
                }
                else {
                    document.getElementById('Ordinary').style.display = 'none';
                    document.getElementById('Bulkd').style.display = 'block';
                }
            }
}
...
//	console.log(max);
    ...
    x.setAttribute("id", 'Text'+m);
	x.setAttribute("class", 'files');
	...
	function removed() {
z=m-1;
if(document.getElementById("Text"+z))
{
var elem = document.getElementById("Text"+z);
...
//	console.log(max);
	...
	var x = document.createElement("INPUT");
    x.setAttribute("type", "text");
    x.setAttribute("id", 'Txt_reportname'+r);
	x.setAttribute("class", 'folders');
	...
	function removed2() {
z=r-1;
if(document.getElementById("Txt_reportname"+z))
{
var elem = document.getElementById("Txt_reportname"+z);
  elem.parentNode.removeChild(elem);
