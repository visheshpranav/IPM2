...
def recursive_csv_list(xml_file_path, master_df, filter_master_df,patterns):
...
##    print(master_df)
    return master_df, filter_master_df,patterns
...
##        old_report =""
    ...
    ##    print(files)
        ...
        each_percent = int(total_count/100)
        ...
        for file in files:
            counter+=1
    ##        print(file)
            master_df, filter_master_df, patterns = recursive_csv_list(file, master_df, filter_master_df,patterns)
            if counter%each_percent == 0:
##                str(int((counter/total_count)*100))
                with open(DATA_PATH + "Cache\\" + userid + "\\" + test_name+"\\Status.txt",'w') as fp:
                    fp.write(str(int((counter/total_count)*100)))
