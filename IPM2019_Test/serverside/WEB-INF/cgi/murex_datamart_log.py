...
def generate_unique_values(datamart_df, column_details_df, final_df):
    ...
    cols = set()
    for value in recursive_items(patterns):
        cols.add(value)
    cols.remove('DM Reports')
    cols.remove('DM Table')
    if 'TIME' not in datamart_df.columns:
        datamart_df['FREQUENCY'] = 0
        group_by_report_df = datamart_df.groupby('REPORT_TABLE')
        for df in group_by_report_df:
            frequency = []
            unique_dates = df[1]['JOB_DATE'].unique()
            for date in unique_dates:
                unique_date_df = df[1].loc[df[1]['JOB_DATE']==date]
                frequency.append(len(unique_date_df))
            datamart_df.loc[datamart_df['REPORT_TABLE']==df[0],'FREQUENCY'] = int(sum(frequency)/len(frequency))
            # print(datamart_df.loc[datamart_df['REPORT_TABLE']==df[0],'FREQUENCY'])
        datamart_df['FREQUENCY'] = datamart_df['FREQUENCY'].astype(str)
    
    else:
        datamart_df['FREQUENCY'] = datamart_df['TIME'].astype('int32')
        datamart_df['FREQUENCY'] = round(datamart_df['FREQUENCY']/3600,1)
    datamart_df.fillna('Not Available', inplace=True)
    datamart_df.replace('','Not Available', inplace=True)
    datamart_df.replace({'CATEGORY': {'Not Available':'Undefined'},'BATCH OF EXTRACTION': {'Not Available':''},
                         'BATCH OF FEEDER': {'Not Available':''}}, inplace=True)
    for column_name in cols:
        if column_name == 'COLUMNS':
            column_details_df = column_details_df.loc[column_details_df['REPORT_TABLE'].isin(datamart_df['REPORT_TABLE'].unique())]
            unique_values = list(column_details_df['COLUMNS'].unique())
        else:
            unique_values = list(datamart_df[column_name].unique())
            unique_values = [val for val in unique_values if val in final_df.values]
            unique_values = [val for val in unique_values if val or val !='nan']
        unique_values_output[column_name] = unique_values
    return unique_values_output
...
def generate_dendo(datamart_df,column_details_df,output_path):
    ...
    if 'JOB_DATE' in datamart_df.columns:
        datamart_df['FREQUENCY'] = 0
        datamart_df['JOB_DATE'] = pd.to_datetime(datamart_df['JOB_DATE']).dt.strftime('%d/%m/%Y')
        frequency = {}
        group_by_report_df = datamart_df.groupby('REPORT_TABLE')
        for df in group_by_report_df:
            # print(frequency.keys())
            if df[0] not in frequency.keys():
                frequency[df[0]] = {}
            unique_dates = df[1]['JOB_DATE'].unique()
            for date in unique_dates:
                unique_date_df = df[1].loc[df[1]['JOB_DATE']==date]
                frequency[df[0]][date] = len(unique_date_df)
            
            datamart_df.loc[datamart_df['REPORT_TABLE']==df[0],'FREQUENCY'] = int(sum(frequency[df[0]].values())/len(frequency[df[0]].values()))
            # print(datamart_df.loc[datamart_df['REPORT_TABLE']==df[0],'FREQUENCY'])
        datamart_df['FREQUENCY'] = datamart_df['FREQUENCY'].astype(str)
        # json.dump(frequency,open(output_path+'//frequency.json','w'))
                    ...
                    if val_level6 != '':
                        ...
                        if flag:
                            if each_group[1] == 'FREQUENCY':
                                # print(row[0][-1])
                                # print(len(final_val))
                                for key,value in frequency[row[0][-1]].items():
                                    dum_final_val = copy.deepcopy(final_val)
                                    dum_final_val.extend([each_group[1],key,value,'',''])
                                    dum_final_val.extend([len(count_df)])
                                    final_list.append(dum_final_val)
                                    # print(key,value)
                            else:
                            ...
                            if each_group[1] == 'FREQUENCY':
                                ...
                                if not len(sub_val):
                                    prev_sub_val[0]["children"].append({"name": each_group[1],"tag": "Node"+str(i+2),"value": val_level6, "children": []})
                                    sub_val = [val for val in prev_sub_val[0]["children"] if val['name'] == each_group[1] and val['value'] == val_level6]
                                for key,value in frequency[row[0][-1]].items():
                                    sub_val[0]["children"].append({"name": key,"tag": "Node"+str(i+3),"value": value, "Count": len(count_df)})
...
##    test_name="nov10_12"	
    logtype="DATAMART"
    # file_path = "C:\\Users\\v.a.subhash.krishna\\Desktop\\Deliverable\\DataMart_Input\\Datamart_R_28042021_100k_edited.csv"
    ...
    try:
        ...
        try:
            final_df = generate_dendo(datamart_df, column_details_df, output_path)
            unique_values_output = generate_unique_values(datamart_df, column_details_df, final_df)
            json.dump(unique_values_output, open(output_path+'//datamart_unique_values.json','w'))
