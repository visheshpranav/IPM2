...
def df_gen(status_count,output_path):
    ...
    final_df = pd.DataFrame(columns = ['Category','Level1','Level2','Level3',
...
##    print(status_count)
    for val in status_count:
        temp_df = pd.DataFrame(columns = ['Category','Level1','Level2','Level3',
        ...
        if not len(df1):
            cors_values = [val1 for val1 in status_count if val1[:4] == main_list]
            completed_columns = {}
            column_count = {}
            for val1 in cors_values:
                if val1[4] not in completed_columns:
                    completed_columns[val1[4]] = {}
                    column_count[val1[4]] = 0
                completed_columns[val1[4]][val1[5]] = val1[6]
                column_count[val1[4]] += val1[6]
            max_val = 0
            max_key = ''
            ...
            for col_val in completed_columns[max_key]:
                new_row={'Category':val[0],'Level1':val[1],'Level2':val[2],'Level3':val[3],max_key:col_val}
...
##                        print(temp_df[col_name])
    ...
    final_df.to_excel(writer,sheet_name='Sheet2',index=False)  #convert to Excel sheet1
...
if __name__ == '__main__':	
    ...
    print(logtype)	
    xlsx_filename='murex_master_data.xlsx'                      #Excel Filename
...
##    print(files)	
    ...
    my_df_new = pd.DataFrame(csv_list, columns = ['Category','Family','Group','Typology','Trade Characteristics','Trade Characteristics Value','Count'])
    ...
    my_df_new.to_excel(writer,sheet_name='Sheet1',index=False)  #convert to Excel sheet2
    writer.save()                                               #Saving Excel sheet
