...
##    typology = typology.replace(' ','_')
    if logtype == 'TradeInsertion':
        ...
        if typology == 'Common Tags':
            ...
            if not view_pattern:
                final_list.append(['Events',patterns_json['lastContractEventReference']])
            else:
                if 'lastContractEventReference' in patterns_json['Dendo_list']:
                    final_list.append(['Events',patterns_json['lastContractEventReference'],'Both'])
                else:
                    final_list.append(['Events',patterns_json['lastContractEventReference'],'Download'])
